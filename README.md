...
> **AI를 나중에 켰다면** 그동안 쓴 메모에는 의미 색인이 없습니다. 앱은 시작할 때마다 색인이 없는
> 메모를 25개씩 따라잡으므로, 메모가 많으면 며칠에 걸쳐 채워집니다. 키워드 검색은 그동안에도
> 전부 정상입니다.
...
**AI 전용 설정 화면은 아직 없습니다.** 값은 메모 데이터베이스의 `settings` 표에 있고, 지금은
SQLite 편집기로 바꿉니다.

1. DeskNote를 **완전히 종료**합니다 (트레이 우클릭 → 종료).
2. [DB Browser for SQLite](https://sqlitebrowser.org) 같은 도구로
   `%LOCALAPPDATA%\DeskNote\notes.db` 를 엽니다.
3. **Execute SQL** 에 아래를 넣고 실행한 뒤 저장(`Write Changes`)합니다.

```sql
INSERT INTO settings(key, value) VALUES ('ai.model', 'llama3.2:3b')
...
- **임베딩 모델을 바꾸면** 이전 모델로 만든 벡터는 비교에서 빠지고 다시 만들어집니다
  (시작할 때마다 25개씩). 서로 다른 모델의 벡터를 섞어 비교하는 일은 없습니다.
- **`ai.endpoint` 에 다른 PC의 주소를 넣으면** 그 순간부터 메모 본문이 네트워크로 나갑니다.
  이 앱이 "밖으로 나가지 않는다"고 말하는 근거는 이 값이 loopback이라는 것뿐입니다. 바꾸는 것은
  가능하지만, 무엇을 포기하는지 알고 바꾸세요.
...
| 의미 검색만 안 맞는다 | 임베딩 모델이 없거나 색인이 덜 됨 | `ollama pull bge-m3` 후, 시작할 때마다 25개씩 채워집니다 |
