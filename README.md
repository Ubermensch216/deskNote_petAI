...
바로 오류로 끝난다. 현재 318개 테스트가 모두 통과한다.
...
노트 상단 막대에서 색상(`NoteColors` 의 8종), 불투명도 슬라이더, 항상 위 고정을 바꾼다. `⋯` 메뉴의
크기 프리셋은 Small · Medium · Large 세 가지다. 셋 다 노트마다 저장되므로 다시 켜도 그대로 뜬다.
불투명도에는 하한(`Note.MinOpacity`)이 있어 노트를 완전히 투명하게 만들어 잃어버리는 일은 없다.

`✦` 버튼은 AI 자리이며 지금은 비활성이다. 로컬 모델이 없으면 누를 것도 없다는 사실을 UI에서 그대로
보여주는 편이, 눌렀을 때 실패하는 버튼보다 낫다.
