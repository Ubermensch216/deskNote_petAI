...
| `src/DeskNote.Companion.Core` | net10.0 | 펫 도메인 — 활동 분류, 밸런스, 성장 사다리, 상태 투영, 종별 대사 코퍼스 |
...
| `tests/DeskNote.Companion.Core.Tests` | net10.0 | 점수·상한·성장 단계·상태 투영의 결정론, 대사 코퍼스와 셔플 덱 |
...
본문 문자열의 줄바꿈은 앱 전체에서 LF 하나다. 저장, 체크리스트 파싱, 제목 추출, FTS가 전부 그것을
기준으로 쓰여 있다. 그런데 리치 에디트 문서는 문단을 **CR** 로 나누므로 `NoteEditor` 가 경계에서 한 번
변환하고(`ForEditor`), 읽을 때 되돌린다(`ReadShown`).

바깥에서 들어오는 텍스트는 CRLF다. 그래서 캐럿에 텍스트를 넣는 경로(`InsertAtCaret`)가 들어오는 값을
`NoteContent.NormalizeLineEndings` 로 먼저 정규화한다. 붙여넣기, 첨부 링크 삽입, 명령 팔레트의 새 메모가
모두 이 한 곳을 지난다.

정규화가 없으면 CR과 LF 두 글자가 문단 마크 두 개가 되어 **줄마다 빈 줄이 하나씩 따라붙는다.**
붙여넣은 목록이 두 배 길이로 벌어지는 것이 그 증상이었다. `ForEditor` 도 같은 정규화를 한 번 더 거는데,
다른 경로로 CRLF가 새어 들어와도 문단 마크가 두 개가 되지 않게 하는 방어선이다. CR이 없는 문자열은
원본을 그대로 돌려주므로 타이핑 경로에 비용이 없다.
...
**이미지는 메모 위에 이미지로 그려진다.** 편집기는 `RichEditBox` 라 강조는 그릴 수 있어도 그림은
그리지 못하므로 편집기 위쪽에 이미지 스트립이 따로 있고, 창이 열릴 때 첨부 레코드에서 복원된다.
전에는 본문에 `![shot.png](C:/Users/…/a1b2….png)` 를 써 넣었고, 그러면 스티키 메모가 사용자가 그림을 기대한
