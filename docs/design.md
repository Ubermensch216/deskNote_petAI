...
`KeyboardAccelerator` 목록이 아니라 `PreviewKeyDown`에서 처리한다. TextBox 안의 편집 컨트롤이
`Ctrl+X`·`Ctrl+C`를 Shift와 상관없이 잘라내기·복사로 읽고 가속기 처리 전에 키를 먹어버려서, 문서에
적힌 취소선 단축키가 선택 영역에 `~~`를 두르는 대신 잘라내고 있었다. `PreviewKeyDown`은 그보다 먼저
터널링하므로 명령이 실행되고 편집 컨트롤은 키를 보지 못한다 — 목록 이어쓰기용 Enter를 여기서 잡는
것과 같은 이유다.
