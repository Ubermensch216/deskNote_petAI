...
글자만 받는 것으로는 부족하고 **줄바꿈도 메모의 것으로 바꿔서 받는다.** 바깥에서 온 텍스트는 CRLF인데
리치 에디트는 CR 하나를 문단 구분으로 읽으므로, 그대로 넣으면 문단 마크가 두 개가 되어 줄마다 빈 줄이
하나씩 따라붙는다. 붙여넣은 목록이 두 배 길이로 벌어지는 것이 그 증상이었다. 정규화가 어디서 일어나고
왜 한 곳이어야 하는지는 [architecture.md](architecture.md#줄바꿈은-lf-하나다)에 있다.
...
편집기는 `RichEditBox` 다. 글자의 강조는 그릴 수 있어도 그림은 그리지 못하므로, 편집기 위에 이미지
...
펫 대시보드와 `펫 키우는 방법`, 그리고 `설정` 은 자기들끼리 하나의 언어를 쓴다 — 채도 높은 히어로
띠, 앱 활동은 남색, 돌봄은 청록, 목표와 미션은 금색. 게임 화면이 시스템 회색을 빌려 쓰면 동물이
들어 있는 설정 페이지로 읽히고, 사용자는 펫 창에서 다른 창으로 넘어가면서 다른 앱에 온 것처럼
느낀다. 셋 중 마지막으로 남아 있던 것이 설정이었고, 바로 다음 절이 그것을 어떻게 고쳤는지다.

색을 고르는 규칙과 그렇게 정한 이유는 [companion.md](companion.md#대시보드는-세-가지-질문이다)에 있다.

## 설정은 캐릭터 화면이다

설정 창은 대시보드의 톱니바퀴에서 열린다. 그런데 그 안이 시스템 회색 폼이면, 방금 게임 화면에 있던
사람이 갑자기 다른 앱에 온 것처럼 느낀다. 그래서 설정도 같은 언어를 쓴다 — 같은 히어로 띠, 펫 자신은
남색, 펫이 하는 일은 청록, 조용히 있는 시간은 보라.

<table>
<tr>
<td><img src="images/settings-character.png" alt="설정 · 캐릭터" width="380"></td>
<td><img src="images/settings-behavior.png" alt="설정 · 동작과 방해 금지 시간" width="380"></td>
</tr>
</table>

말로 하던 것 세 가지가 그림이 됐다.

| 전 | 후 |
|---|---|
| 펫 선택 드롭다운 | 초상화 일곱 장. 무엇을 가졌는지 보려고 목록을 열지 않아도 되고, 잠긴 용은 초상화 위에 자물쇠를 달고 있다 |
| `작게 / 중간 / 크게` 드롭다운 | 크기 세 개. 고르면 히어로 띠의 펫이 그 크기로 바뀐다 |
| 시각 두 개 | 하루를 가로지르는 띠. 자정을 넘는 구간은 **양쪽 끝** 이 칠해지는데, 시계 두 개로는 그게 보이지 않았다 |

이름·종·크기는 모두 히어로 띠에 바로 반영된다. 저장하기 전에 무엇을 고른 것인지 볼 수 있어야 고른
것이다.

`동반자 사용` 이 꺼지면 나머지 스위치 셋과 방해 금지 시간이 흐려진다. 셋 다 동반자가 켜져 있어야
의미가 있고, 방해 금지 시간은 애초에 `먼저 제안하기` 만 막는 값이다. 살아 있는 채로 두면 절대 적용될
수 없는 시간대를 정성껏 설정하게 된다. 흐려질 뿐 값은 그대로이므로 저장으로 잃는 것은 없다.

`저장` 은 아래에 고정돼 있다. 페이지가 한 화면보다 길어서, 바닥까지 스크롤하지 않으면 바뀐 것이
사라지는 설정 창이 될 뻔했다.
