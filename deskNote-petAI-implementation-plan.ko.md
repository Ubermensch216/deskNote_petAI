...
## 0. 진행 상황 (2026-09-08 기준)

> 아래 계획 본문은 2026-08-31에 세운 **기준서**이며 그대로 보존한다. 이 절만 현재 상태를 기록한다.
> 지금 동작하는 것의 설명은 계획서가 아니라 [docs/architecture.md](docs/architecture.md),
> [docs/companion.md](docs/companion.md), [docs/design.md](docs/design.md)에 있다.

| 로드맵 | 상태 |
|---|---|
| Phase 0 — 기준선과 저장소 위생 | 완료 ([docs/phase0-baseline.md](docs/phase0-baseline.md)) |
| Phase 1 — 컴패니언 접점 | 완료 ([docs/phase1-progress.md](docs/phase1-progress.md)) |
| Phase 2 — 게임 도메인과 저장 | 완료 ([docs/phase2-3-progress.md](docs/phase2-3-progress.md)) |
| Phase 3 — 컴패니언 UI와 접근성 | 완료. 이후 대시보드·안내 화면을 다시 설계했다 |
| Phase 4 — 회상 루프와 선제성 | 완료 |
| Phase 5 — 베타, 밸런싱, 배포 | 진행 중 (win-x64 폴더형 패키지 자동화까지 완료) |

계획과 달라진 결정은 다음과 같다. 계획 본문의 해당 서술보다 이 목록이 우선한다.

1. **성장 밸런스를 30:70으로 바꿨다.** 계획의 초기 배점은 하루 최대 29점에 그쳐 펫이 앱 사용
   영수증에 가까웠다. 앱 활동 30점 + 직접 돌봄 70점으로 옮겼다 ([docs/companion.md](docs/companion.md)).
2. **삭제에서 휴지통 단계를 없앴다.** 삭제는 즉시·영구이고 확인 대화상자가 그 자리를 대신한다.
   저장 계층의 소프트 삭제 기능은 쿼리 필터 테스트용으로만 남아 있다.
3. **시작 화면 정책을 세웠다.** 열려 있던 메모를 복원하고, 없으면 최근 메모 한 장을 연다.
   라이브러리를 여는 분기는 제거했다.
4. **첨부 이미지를 메모 위에 그린다.** 본문에 Markdown 경로를 써 넣던 방식을 대체했다.
5. **노트북 이름 변경·삭제·메모 이동 UI를 추가했다.** 데이터 계층에는 이미 있던 기능이다.
6. **펫 대사를 종별 300개로 늘렸다.** 계획에는 대사 분량이 없었고 구현은 종 구분 없는 12개로
   시작했는데, 종을 바꿔도 같은 말을 하니 펫을 고르는 일이 색깔 고르기와 다르지 않았다. 지금은
   7종 × 2언어 × 300줄이며 UI 리소스가 아니라 `DeskNote.Companion.Core`의 임베드 코퍼스에 있다.
   뽑는 방식도 무작위 추첨에서 셔플 덱으로 바꿨다 ([docs/companion.md](docs/companion.md#혼잣말은-종마다-다르다)).
7. 계획의 `상점·아이템·인벤토리`는 여전히 범위 밖이다.

### 최근 수정 (2026-09-08)

| 증상 | 원인 |
|---|---|
| 펫이 다른 창에 가려짐 | 창을 띄우기 전에 준 항상-위 설정이 WinUI에서 창 관리자까지 가지 않았다. 첫 활성화 이후 승격 + 네이티브 `SetWindowPos`로 고쳤다 |
| 붙여넣기 시 줄마다 빈 줄 | 클립보드의 CRLF를 정규화 없이 넣어 문단 마크가 두 개가 됐다. 캐럿 삽입 경로 한 곳에서 LF로 정규화한다 |
